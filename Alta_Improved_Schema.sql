-- Alta Media Web App - Improved Database Schema
-- Enhanced with better user management and invitation system
...
CREATE TABLE companies (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, -- Company owner
...
-- User Invitations table (for inviting people to join companies)
...
-- View for user's company access
CREATE VIEW user_company_access AS
SELECT 
    u.id as user_id,
    u.email,
    u.full_name,
    ...
    cu.is_active as company_access_active,
    cu.joined_at
FROM users u
...
WHERE ui.status = 'pending' AND ui.expires_at > CURRENT_TIMESTAMP;
